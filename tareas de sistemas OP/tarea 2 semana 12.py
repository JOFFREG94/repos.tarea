...
# Creamos un objeto mutex (candado)
mutex = threading.Lock()

# Función que incrementa el contador global de forma segura utilizando un mutex
def incrementar():
    global contador_global
    # Adquirimos el mutex (candado) para acceder a la sección crítica
    mutex.acquire()
    try:
        # Sección crítica: Incrementamos el contador
        contador_global += 1
    finally:
        # Liberamos el mutex para que otros hilos puedan acceder a la sección crítica
        mutex.release()
